...
def aroon_bbands_ema(data, freq, aroon_window=35, bbands_window=20, bbands_dev_up=3, bbands_dev_down=3, ema_window_long=200, ema_window_short=90):
    
    df=data.copy()
        
    df = df.resample(f"{freq}min").agg({"Open": "first", "High": "max", "Low": "min", "Close": "last", "vol": "sum", 'spread':'mean','pips':'mean'}).dropna()
    ...
    df['upperband'], df['middleband'], df['lowerband']=ta.BBANDS(df['Close'], bbands_window, bbands_dev_up, bbands_dev_down)
    ...
    conditions_ema=[(df['EMA_short']>df['EMA_long'])&(df['EMA_short_shift']<df['EMA_long_shift']),(df['EMA_short']<df['EMA_long'])&(df['EMA_short_shift']>df['EMA_long_shift'])]
    ...
    conditions_comb=[(df['position_aroon']==1)&(df['position_ch_bbands']==1),(df['position_aroon']==-1)&(df['position_ch_bbands']==-1),df['position_ch_ema']!=0]
